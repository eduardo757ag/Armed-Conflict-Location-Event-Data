...
DATA <- merge(ACLED, GDP, by=c("year","country"))
```
...
DATA$EVENT_TYPE <- as.numeric(DATA$count)
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "STRATEGIC DEVELOPMENT"] <- "ARMED GROUP CONFLICT"
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "BATTLE-NO CHANGE OF TERRITORY"] <- "ARMED GROUP CONFLICT"
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "BATTLE-GOVERNMENT REGAINS TERRITORY"] <- "ARMED GROUP CONFLICT"
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "BATTLE-NON-STATE ACTOR OVERTAKES TERRITORY"] <- "ARMED GROUP CONFLICT"
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "NON-VIOLENT TRANSFER OF TERRITORY"] <- "ARMED GROUP CONFLICT"
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "REMOTE VIOLENCE"] <- "ARMED GROUP CONFLICT"
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "VIOLENCE AGAINST CIVILIANS"] <- "CIVILIAN INVOLVED CONFLICT"
DATA$EVENT_TYPE[DATA$EVENT_TYPE == "REMOTE VIOLENCE"] <- "CIVILIAN INVOLVED CONFLICT"

DATA <- DATA %>% count(letter, number) %>% ungroup %>% group_by(letter) %>% mutate(sum.n = sum(n))
