...
ACLED %>% filter(Country == Congo (Democratic Republic of the)) %>%  rename(Congo (Democratic Republic of the)) Democratic Republic of Congo)
...
```{r}
DATA <- cbind(DATA, dummy(DATA$EVENT_TYPE)) #dummy creation

HDI <- rename(HDI, c(Congo (Democratic Republic of the) = "Democratic Republic of Congo"))

table <- table(ACLED$Country, ACLED$count)
table1 <- table(DATA$country, DATA$count)
View(table1)
```
