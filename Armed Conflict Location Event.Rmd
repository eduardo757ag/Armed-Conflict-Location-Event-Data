...
ACLED2 <- ACLED %>% 
  mutate(dep1 = ifelse(EVENT_TYPE %in% cat_social_conflict, "social.conflict", "military.conflict" ))
ACLED2 <- ACLED2 %>% 
  ungroup() %>%
  dplyr::group_by(Country, year, dep1) %>%
  dplyr::summarise(dep2 = sum(count)) %>%
  spread(dep1, dep2)
names(ACLED2)[names(ACLED2)== "Country"] <- "country"
ACLED2 <- apply(ACLED2,2,toupper)
...
DATA$lnmilitary <- log(DATA$military.conflict)
DATA$social.conflict <- as.numeric(as.character(DATA$social.conflict))
DATA$lnsocial <- log(DATA$social.conflict)
...
#facebook$country <- apply(facebook,2,toupper)
#facebook$country <- swap(facebook$country ,"CONGO"="Republic of Congo")
#facebook$country <- dplyr::recode(facebook$country ,"CONGO, Democratic Republic of the, #(ex-ZAIRE)"="Democratic Republic of Congo")
#facebook$country  <- dplyr::recode(facebook$country ,"COTE D'IVOIRE (IVORY COAST)"="Ivory #Coast")
#facebook$country <- dplyr::recode(facebook$country ,"TANZANIA"="Tanzania")
#facebook$country <- apply(facebook,2,toupper)
